...
async def optimize_content(
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an SEO expert providing detailed content analysis."},
...
async def run_onpage_audit(
    ...
    try:
        ...
        1. overall_score (0-100)
        2. technical_issues (array of issues with severity: critical/warning/info)
        3. recommendations (array of actionable improvements)
        4. meta_tags_analysis (title, description quality)
        5. heading_structure (H1-H6 analysis)
        6. image_optimization (alt tags, file sizes)
        7. internal_linking (structure quality)
        8. mobile_friendliness (score 0-100)
        9. schema_markup (present/missing)
        10. page_speed_insights (use score: {pagespeed_score})
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert technical SEO auditor."},
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to parse audit data"
        )
    except Exception as e:
        if connection:
            connection.rollback()
...
async def suggest_backlinks(
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert SEO outreach specialist."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.8,
            max_tokens=800
        ...
        email_draft = json.loads(response.choices[0].message.content)
...
async def optimize_for_voice_search(
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a voice search and semantic SEO expert."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
            max_tokens=1500
